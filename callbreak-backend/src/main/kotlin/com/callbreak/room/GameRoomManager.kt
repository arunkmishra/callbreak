...
object GameRoomManager {
     ...
     */
    fun createRoom(playerName: String, totalRounds: Int = 5): Triple<String, String, String> {
        val roomId = generateUniqueRoomId()
        val playerId = UUID.randomUUID().toString()
        val sessionToken = UUID.randomUUID().toString()
        val host = Player(id = playerId, name = playerName)
        val initialState = CallbreakState(roomId = roomId, players = listOf(host), totalRounds = totalRounds)
