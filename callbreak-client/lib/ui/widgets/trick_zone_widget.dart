...
class _TrickZoneWidgetState extends State<TrickZoneWidget> {
  ...
  /// The final resting offset of the card relative to center (90x90 container).
  Offset _restOffsetForRelativeIndex(int relativeIndex) {
    switch (relativeIndex) {
      case 0:
        return const Offset(0, 35); // bottom (me)
      case 1:
        return const Offset(-35, 0); // left
      case 2:
        return const Offset(0, -35); // top
      case 3:
        return const Offset(35, 0); // right
      default:
        return Offset.zero;
    }
  }

  /// The starting offset (off-screen edge) for the fly-in animation.
  Offset _startOffsetForRelativeIndex(int relativeIndex) {
    switch (relativeIndex) {
      case 0:
        return const Offset(0, 90); // flies in from bottom
      case 1:
        return const Offset(-90, 0); // flies in from left
      case 2:
        return const Offset(0, -90); // flies in from top
      case 3:
        return const Offset(90, 0); // flies in from right
      default:
        return Offset.zero;
    }
  }
